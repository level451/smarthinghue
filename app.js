...
//hue.lights[1].state.xy = [0.0000,0.0000];
//hue.lights[1].state.ct = 0;
//hue.lights[1].state.alert = "none";
//hue.lights[1].state.effect = "none";
//hue.lights[1].state.colormode = "hs";
//hue.lights[1].state.reachable = true;
//hue.lights[1].modelid ="LCT001";
...
client.on('message', function (msg, remote) {
   // console.log('upnp messge');
//    if (remote.address == "10.6.1.59"){
//
//    console.log('Smartthings UPnP Broadcast received.');
//   // console.log('From: ' + remote.address + ':' + remote.port +' \n\n' + msg);
//
//    }

    if (msg.toString().indexOf("urn:schemas-upnp-org:device:basic:1") > -1 ){
      //  console.log('Hue UPnP Broadcast received.');
       ...
       // console.log(req);
        ...
        req.on('end', function() {
            if ((req.url.indexOf("lights")) > 0){
               ...
               if (requestedState.on == true) {
                   res.write(JSON.stringify([{"success": {"/lights/1/state/on":true}}]));
               } else
               {
                   res.write(JSON.stringify([{"success": {"/lights/1/state/on":false}}]));
               }

                res.end();
                console.log('Changeing'+req.url+' - '+fullBody+' = '+requestedState.on);
